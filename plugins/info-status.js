...
let handler = async (m, {
    ...
    try {
        let { self, autoread, restrict, getsw, publicJoin, unavailable, statusUpdate, mature, autotyping, status, readsw } = db.data.settings[conn.user.jid]
...
Runtime : *${uptime}*`
        conn.sendButton(m.chat, txt, wm, null, [["Menu", "#menu"]], m)
        console.log(OS)
    } catch (e) {
        console.log(e)
        m.replyv2(eror)
