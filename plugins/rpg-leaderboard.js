...
let handler = async (m, { conn, args, participants }) => {
  ...
  let len = args[0] && args[0].length > 0 ? Math.min(100, Math.max(parseInt(args[0]), 3)) : Math.min(5, sortedExp.length)
  let text = `
...
│ 
${sortedMon.slice(0, len).map(({ jid, money }, i) => `├ ${i + 1}. ${participants.some(p => jid === p.jid) ? `${db.data.users[jid] ? db.data.users[jid].name : conn.getName(jid)}` : `${db.data.users[jid] ? db.data.users[jid].name : conn.getName(jid)}`} *${money} Money*`).join`\n`}
└────
