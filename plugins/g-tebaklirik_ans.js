...
handler.before = async function (m) {
    ...
    if (!(id in this.tebaklirik)) return m.replyv2('Soal itu telah berakhir')
    if (m.quoted.id == this.tebaklirik[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!* +${this.tebaklirik[id][2]} XP`, wm, null, [['Tebak Lirik', '.tebaklirik']], m)
