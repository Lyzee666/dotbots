...
export async function handler(chatUpdate) {
    ...
    global.image = await this.buffer(`${global.img},dobdabot`)
    global.uptime = func.clockString(process.uptime() * 1000)
    global.linkgc = func.mathRandom(['https://chat.whatsapp.com/L508viIjjPwHpYEqgTxqj6', 'https://chat.whatsapp.com/JsDLZeKb0YLDyrYc1L28sk'])
    global.numb = func.mathRandom([400000271, 400000271])
    global.done = func.mathRandom(["Sukses Sayang.. ", "Done", "Sukses"])
    global.eror = func.mathRandom([
        'Error',
        'Astagfirullah Error',
        'Nice Error',
    ...
    try {
         ...
         */
        if (db.data.settings[this.user.jid].readsw) this.chatRead('status@broadcast', m.key.participant, m.id || m.key.id).catch(() => {})
